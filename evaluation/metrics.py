...
def calculate_metrics(predictions, labels, config):
    ...
    for index in tqdm(predictions.keys(), ncols=80):
        ...
        np.save(f"/home/zhixin_yan/project/RhythmMamba/output/rPPGandLabel/prediction_{index}.npy",prediction)
        np.save(f"/home/zhixin_yan/project/RhythmMamba/output/rPPGandLabel/label{index}.npy",label)
    ...
    if config.INFERENCE.EVALUATION_METHOD == "FFT":
        ...
        print(f"gt_hr_peak_all:{gt_hr_fft_all}")
        predict_hr_fft_all = np.array(predict_hr_fft_all)
        print(f"predict_hr_peak_all:{predict_hr_fft_all}")
        SNR_all = np.array(SNR_all)
        print(f"SNR_all:{SNR_all}")
